...
namespace PylonC.NETSupportLibrary
{
    ...
    public class ImageProvider
    {
        ...
        public void Setup(int width, int height)
        {
            try
            {
                if (Pylon.DeviceFeatureIsWritable(m_hDevice, "PixelFormat"))
                {
                    Pylon.DeviceFeatureFromString(m_hDevice, "PixelFormat", "Mono8");
                }

                string featureName;  /* Name of the feature used in this sample: AOI Width. */
                long val, min, max, incr;      /* Properties of the feature. */

                featureName = "OffsetX";  /* Name of the feature used in this sample: AOI Width. */
        ...
        /// 露出値の設定([usec])
        /// </summary>
        public double SetupExposureTimeAbs(double expo)
        {
            try
            {
                string featureName;  /* Name of the feature used in this sample: AOI Width. */
                ...
                if (isAvailable)
                {
                    ...
                    if (isWritable)
                    {
                        ...
                        if (val < min) val = min;
                        if (val > max) val = max;
                        Pylon.DeviceSetFloatFeature(m_hDevice, featureName, val);      /* Set allowed value. */
        ...
        /// フレームレートの設定
        /// </summary>
        public double SetupFrameRate(double frate)
        {
            try
            {
                string featureName;  /* Name of the feature used in this sample: AOI Width. */
